...
interface GinStackProps extends StackProps {
  config: GinConfig;
}
...
export class GinStack extends Stack {
  constructor(scope: Construct, id: string, props: GinStackProps) {
    ...
    const anthropicSecret = new secrets.Secret(this, "AnthropicKey", {
      description: "Gin Roam — Anthropic Workspace API key (set value after deploy)",
    });
    const anthropicAdminSecret = new secrets.Secret(this, "AnthropicAdminKey", {
      description: "Gin Roam — Anthropic Admin API key for org cost reporting",
    });
    const vapidSecret = new secrets.Secret(this, "VapidKey", {
      description: "Gin Roam — Web Push VAPID {publicKey, privateKey, subject} (set value after deploy)",
    });
    // Provider creds — swappable by updating the secret value (no redeploy).
    const resendSecret = new secrets.Secret(this, "ResendKey", {
      description: "Gin Roam — Resend {apiKey, fromEmail}. Shareable now; swap value to switch accounts.",
    });
    const twilioSecret = new secrets.Secret(this, "TwilioKey", {
      description: "Gin Roam — Twilio {accountSid, authToken, fromNumber}. Swap value to switch accounts.",
    });
    ...
    if (config.resendCognitoEmails) {
      emailKey = new kms.Key(this, "CognitoEmailKey", {
        description: "Gin Roam Cognito custom email sender",
    ...
    const googleOAuthSecret = new secrets.Secret(this, "GoogleOAuthSecret", {
      description: "Gin Roam — Google OAuth client secret: set { \"clientSecret\": \"...\" }",
    });
    const appleOAuthSecret = new secrets.Secret(this, "AppleOAuthSecret", {
      description: "Gin Roam — Apple Sign in private key (.p8): set { \"privateKey\": \"...\" }",
    });
    ...
    const apiWaf = new wafv2.CfnWebACL(this, "ApiWaf", {
      scope: "REGIONAL",
      defaultAction: { allow: {} },
      visibilityConfig: { cloudWatchMetricsEnabled: true, metricName: "ginApi", sampledRequestsEnabled: true },
      rules: [
        {
          name: "ApiRateLimit",
          priority: 1,
    ...
    // -------------------------------------------------------- Monitoring
    const alarmTopic = new sns.Topic(this, "Alarms", { displayName: "Gin Roam alarms" });

    // App alarms use an "gin-" name prefix so the admin dashboard can list only
    ...
    errAlarm("ApiErrors", "gin-api-errors", apiFn, 5, 5);
    errAlarm("EnrichErrors", "gin-enrich-errors", enrichFn, 3, 15);
    errAlarm("ReminderErrors", "gin-reminder-errors", reminderFn, 2, 15);
    errAlarm("ChatErrors", "gin-chat-errors", chatFn, 5, 15);

    new cw.Alarm(this, "DynamoThrottles", {
      alarmName: "gin-dynamodb-throttles",
