...
function modelDrivers(model: string): string {
  ...
  if (m.includes("sonnet")) return "Gin assistant chat · post-tasting quiz generation";
    ...
    <>
      ...
      <div className="card" style={{ marginTop: 12 }}>
        ...
        <span className="bar-track" style={{ marginTop: 8 }}>
          <span className="bar-fill" style={{ width: `${Math.min(100, pct)}%`, background: pct > 80 ? "var(--amber)" : "var(--gin)" }} />
        </span>
        ...
        {costs.awsByService.length === 0 ? (
          ...
          costs.awsByService.map((s) => (
            <div key={s.name} className="bar-row">
              ...
              <span className="bar-track"><span className="bar-fill" style={{ width: `${(s.amount / svcMax) * 100}%`, background: "var(--gin)" }} /></span>
        ...
        {costs.awsDaily.length === 0 ? (
          ...
          costs.awsDaily.map((d) => (
            <div key={d.name} className="bar-row">
              ...
              <span className="bar-track"><span className="bar-fill" style={{ width: `${(d.amount / dayMax) * 100}%`, background: "var(--gin)" }} /></span>
        ...
        {chart.length === 0 ? (
          ...
          chart.map((d) => (
            <div key={d.day} className="bar-row">
              ...
              <span className="bar-track"><span className="bar-fill" style={{ width: `${(d[metric] / max) * 100}%`, background: "var(--gin)" }} /></span>
            ...
            <div key={u.username} className="card">
              <div onClick={() => expand(u)} style={{ cursor: "pointer", display: "flex", justifyContent: "space-between", alignItems: "baseline", gap: 8 }}>
                ...
                <div style={{ display: "flex", gap: 6, alignItems: "center", flexShrink: 0 }}>
                  {u.isAdmin && <span className="badge" style={{ background: "#e9f0e4", color: "var(--gin-deep)" }}>admin</span>}
...
const ALARM_COLOR = (s: string) => (s === "ALARM" ? "#b23b2c" : s === "OK" ? "var(--gin)" : "var(--amber)");
            ...
            return (
              <div key={`${r.at}-${i}`} className="card">
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", gap: 8 }}>
                  <span className="badge" style={{ background: danger ? "#fcefec" : "#e9f0e4", color: danger ? "#b23b2c" : "var(--gin-deep)" }}>
...
const FB_COLOR: Record<string, { bg: string; fg: string }> = {
  ...
  Idea: { bg: "#e9f0e4", fg: "var(--gin-deep)" },
        ...
        <div className="card stack" style={{ marginTop: 8 }}>
          {rows.map((r) => (
            <div key={r.page} className="bar-row">
              ...
              <span className="bar-track"><span className="bar-fill" style={{ width: `${(r.count / max) * 100}%`, background: "var(--gin)" }} /></span>
        ...
        {list.map((b) => (
          <div className="card" key={b.id}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline" }}>
              ...
              <span className="badge" style={b.verified ? { background: "#e9f0e4", color: "var(--gin-deep)" } : { background: "#fdeede", color: "var(--amber)" }}>{b.verified ? "verified" : "pending"}</span>
            </div>
        ...
        {items.map((r) => {
          ...
          return (
            <div className="card" key={key(r)}>
              ...
              {a && (
                <div className="badge" style={{ marginTop: 10, background: flagged ? "#fcefec" : "#e9f0e4", color: flagged ? "#b23b2c" : "var(--gin-deep)" }}>
              ...
              <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
                ...
                <button className="btn ghost" style={{ marginLeft: 0, flex: 1, padding: 10, color: "var(--gin-deep)" }} disabled={busy === key(r)} onClick={() => decide(r, "approve")}>Approve</button>
